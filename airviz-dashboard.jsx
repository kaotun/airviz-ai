...
// ─── OVERVIEW TAB ─────────────────────────────────────────────────────────────
const OverviewTab = () => {
  const days30 = generate30Days();
  const pieData = [
    {name:"Tốt",value:8},{name:"Trung bình",value:22},{name:"Kém",value:18},
    {name:"Xấu",value:10},{name:"Rất xấu",value:4},{name:"Nguy hiểm",value:1},
  ];
  const months = ["T1","T2","T3","T4","T5","T6","T7","T8","T9","T10","T11","T12"];
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:32}}>
      {/* KPIs */}
      <section>
        <SectionHeader title="Tổng quan quốc gia" sub="Cập nhật lần cuối: 27/06/2026 08:30 ICT"/>
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:16}}>
          <KPICard label="AQI Trung bình" value="42.3" color={C.sky} icon="🌤️"/>
          <KPICard label="PM2.5 Trung bình" value="18.7" unit="µg/m³" color={C.warning} icon="💨"/>
          <KPICard label="Tỉnh vượt ngưỡng" value="12/63" color={C.danger} icon="⚠️"/>
      ...
      {/* 30-day AQI trend */}
      <section>
        <SectionHeader title="AQI Trung bình Quốc gia — 30 ngày" sub="Đường WHO (PM2.5 = 25 µg/m³) hiển thị màu vàng"/>
        <div style={{...glassCard,padding:"24px 20px"}}>
          <div style={{display:"flex",gap:20,marginBottom:16,flexWrap:"wrap"}}>
            {[{c:C.sky,l:"AQI quốc gia"},{c:C.warning,l:"WHO 25 µg/m³"},{c:C.muted,l:"Mùa khô"}].map(({c,l})=>(
          ...
          <ResponsiveContainer width="100%" height={260}>
            <ComposedChart data={days30} margin={{top:10,right:20,bottom:0,left:0}}>
              <defs>
              ...
              <YAxis tick={{fill:C.muted,fontSize:11}} axisLine={false} tickLine={false} domain={[20,90]}/>
              <Tooltip content={<DarkTooltip/>}/>
              <ReferenceLine y={25} stroke={C.warning} strokeDasharray="5 3" strokeWidth={1.5} label={{value:"WHO",fill:C.warning,fontSize:11,position:"right"}}/>
      ...
      <section>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:20}}>
          <div style={{...glassCard}}>
            <SectionHeader title="Phân phối AQI theo mức độ"/>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6,marginTop:8}}>
              {pieData.map((p,i)=>(
                <div key={i} style={{display:"flex",alignItems:"center",gap:6,fontSize:11,color:C.muted}}>
                  ...
                  {p.name} <span style={{color:C.text,fontWeight:600,...monoFont}}>{Math.round(p.value/63*100)}%</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={{...glassCard}}>
            <SectionHeader title="Top 10 tỉnh ô nhiễm nhất"/>
            <div style={{display:"flex",flexDirection:"column",gap:10,marginTop:8}}>
              {provinces.slice(0,10).sort((a,b)=>b.aqi-a.aqi).map((p,i)=>(
                <div key={i} style={{display:"flex",alignItems:"center",gap:10}}>
                  <span style={{color:C.muted,fontSize:11,width:16,...monoFont}}>{i+1}</span>
                  <span style={{color:C.text,fontSize:13,width:100,...headFont}}>{p.name}</span>
                  <div style={{flex:1,height:6,background:"rgba(255,255,255,0.05)",borderRadius:3,overflow:"hidden"}}>
                    <div style={{height:"100%",width:`${p.aqi/100*100}%`,background:`linear-gradient(90deg,${C.sky},${C.violet})`,borderRadius:3,transition:"width 1s ease"}}/>
                  </div>
                  <span style={{color:aqiColor(p.aqi),fontSize:12,fontWeight:700,width:28,...monoFont}}>{p.aqi}</span>
                </div>
              ))}
            </div>
          </div>
...
const MapTab = () => {
  ...
  const vnProvinces = [
    {name:"Hà Nội",x:230,y:140,aqi:78},{name:"Hải Phòng",x:260,y:150,aqi:72},{name:"Bắc Ninh",x:240,y:130,aqi:68},
    {name:"TP.HCM",x:220,y:420,aqi:65},{name:"Bình Dương",x:210,y:400,aqi:62},{name:"Đà Nẵng",x:230,y:280,aqi:34},
    {name:"Huế",x:220,y:255,aqi:38},{name:"Cần Thơ",x:200,y:440,aqi:45},{name:"Đồng Nai",x:240,y:415,aqi:58},
    {name:"Hưng Yên",x:245,y:155,aqi:65},
  ];
        ...
        <div style={{display:"grid",gridTemplateColumns:"45% 55%",gap:20}}>
          ...
          <div style={{...glassCard,padding:16}}>
            <svg viewBox="0 0 340 550" style={{width:"100%",height:460}}>
              ...
              {vnProvinces.map(p=>(
                <g key={p.name} onClick={()=>setSelected(provinces.find(pp=>pp.name===p.name)||selected)} style={{cursor:"pointer"}}>
                  <circle cx={p.x} cy={p.y} r={selected.name===p.name?14:10} fill={aqiColor(p.aqi)} fillOpacity={0.8} stroke={selected.name===p.name?C.sky:"rgba(255,255,255,0.2)"} strokeWidth={selected.name===p.name?2:0.5}/>
          ...
          <div style={{...glassCard}}>
            <div style={{display:"flex",alignItems:"center",gap:16,marginBottom:20}}>
              <div style={{width:72,height:72,borderRadius:"50%",background:`conic-gradient(${aqiColor(selected.aqi)} 0% ${selected.aqi/200*100}%, rgba(255,255,255,0.05) ${selected.aqi/200*100}% 100%)`,display:"flex",alignItems:"center",justifyContent:"center",position:"relative"}}>
                <div style={{width:56,height:56,borderRadius:"50%",background:C.card,display:"flex",alignItems:"center",justifyContent:"center"}}>
                  <span style={{color:aqiColor(selected.aqi),fontWeight:700,fontSize:18,...monoFont}}>{selected.aqi}</span>
                </div>
              </div>
              <div>
                <h2 style={{color:C.text,fontSize:22,margin:0,...headFont}}>{selected.name}</h2>
                <span style={{background:`${aqiColor(selected.aqi)}22`,border:`1px solid ${aqiColor(selected.aqi)}44`,borderRadius:6,padding:"2px 10px",fontSize:12,color:aqiColor(selected.aqi)}}>{AQI_LABELS[selected.level]}</span>
              </div>
            </div>
            <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10,marginBottom:16}}>
              {[
                {label:"PM2.5",val:`${selected.pm25} µg/m³`,color:C.sky},
                {label:"PM10",val:`${selected.pm10} µg/m³`,color:C.warning},
                {label:"CO",val:"6.2 mg/m³",color:C.violet},
                {label:"NO₂",val:"42 µg/m³",color:"#f97316"},
                {label:"SO₂",val:"18 µg/m³",color:C.success},
                {label:"O₃",val:"68 µg/m³",color:"#a3e635"},
              ].map(({label,val,color})=>(
            ...
            <tbody>
              {provinces.sort((a,b)=>b.aqi-a.aqi).map((p,i)=>(
                <tr key={i} style={{borderBottom:`1px solid rgba(255,255,255,0.03)`,transition:"background 0.2s"}}
                  ...
                  <td style={{padding:"10px 16px",color:C.text,fontSize:13,...headFont}}>{p.name}</td>
                  <td style={{padding:"10px 16px"}}>
                    <span style={{background:`${aqiColor(p.aqi)}18`,color:aqiColor(p.aqi),borderRadius:6,padding:"3px 10px",fontSize:12,fontWeight:700,...monoFont}}>{p.aqi}</span>
                  </td>
                  <td style={{padding:"10px 16px",color:aqiColor(p.aqi),fontSize:12,...headFont}}>{AQI_LABELS[p.level]}</td>
                  <td style={{padding:"10px 16px",color:C.text,fontSize:12,...monoFont}}>{p.pm25} µg/m³</td>
                  <td style={{padding:"10px 16px",color:C.text,fontSize:12,...monoFont}}>{p.pm10} µg/m³</td>
                  <td style={{padding:"10px 16px",color:C.muted,fontSize:12,...headFont}}>{p.region}</td>
                  <td style={{padding:"10px 16px"}}>
                    <span style={{color:p.aqi>42.3?C.danger:C.success,fontSize:12,...monoFont}}>{p.aqi>42.3?"+":""}{(p.aqi-42.3).toFixed(1)}</span>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      <section>
        ...
        <div style={{...glassCard}}>
          <ResponsiveContainer width="100%" height={300}>
            <ComposedChart data={compareData} margin={{top:10,right:20,bottom:0,left:0}}>
              <defs>
                {provs.map(({key,color})=>(
              ...
              <YAxis tick={{fill:C.muted,fontSize:11}} axisLine={false} tickLine={false}/>
              <Tooltip content={<DarkTooltip/>}/>
              {visible.hanoi&&<Area type="monotone" dataKey="hanoi" stroke={C.sky} strokeWidth={2} fill="url(#cghanoi)" dot={false} name="Hà Nội"/>}
              {visible.hcm&&<Area type="monotone" dataKey="hcm" stroke={C.violet} strokeWidth={2} fill="url(#cghcm)" dot={false} name="TP.HCM"/>}
              {visible.danang&&<Area type="monotone" dataKey="danang" stroke={C.emerald} strokeWidth={2} fill="url(#cgdanang)" dot={false} name="Đà Nẵng"/>}
            </ComposedChart>
          </ResponsiveContainer>
        </div>
      </section>
